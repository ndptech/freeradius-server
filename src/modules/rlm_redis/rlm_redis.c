				     ...
				     fr_value_box_list_t *in)
{
	...
	unsigned long				idx = 0;
	fr_value_box_t				*vb;
	fr_value_box_t				*key_vb = fr_dlist_head(in);
	fr_value_box_t				*idx_vb = fr_dlist_next(in, key_vb);

	key = key_vb->vb_strvalue;
...
extern xlat_arg_parser_t redis_node_xlat_args[];
xlat_arg_parser_t redis_node_xlat_args[] = {
	{ .required = true, .concat = true, .variadic = false, .type = FR_TYPE_STRING, .func = NULL, .uctx = NULL },
	{ .required = false, .concat = true, .variadic = false, .type = FR_TYPE_UINT32, .func = NULL, .uctx = NULL },
