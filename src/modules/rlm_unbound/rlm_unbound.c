 ...
 */
static inline void unbound_event_cleanup(unbound_request_t *ur)
{
	if(ur->ev) fr_event_timer_delete(&ur->ev);
}
...
/*
 *	Callback from timeout event.  Cancel the unbound reqeust.
 */
...
/*
 *	Xlat signal callback if the unbound reqeust needs forcibly cancelling
 */
static void xlat_unbound_signal(request_t *request, UNUSED void *instance, UNUSED void *thread, void *rctx, fr_state_signal_t action)
{
	unbound_request_t	*ur = talloc_get_type_abort(rctx, unbound_request_t);

	if (action != FR_SIGNAL_CANCEL) return;

	RDEBUG2("Cancelling pending unbound request");
	ub_cancel(ur->t->ub, ur->async_id);
	unbound_event_cleanup(ur);
}
					 ...
					 UNUSED fr_value_box_list_t *in, void *rctx)
{
	...
	error:
		ub_resolve_free(ur->result);
		return XLAT_ACTION_FAIL;
	}

	if (ur->result->nxdomain) {
		RERROR("%s - Nonexistent domain", xt->inst->name);
		goto error;
	}

	if (!ur->result->havedata) {
	...
	do {
		...
		case FR_TYPE_OCTETS:
			if (fr_value_box_from_network(ctx, vb, ur->return_type, NULL, (uint8_t *)ur->result->data[i], ur->result->len[i], true) < 0) {
			error2:
				talloc_free(vb);
				goto error;
			}
		...
		case FR_TYPE_STRING:
		{
			...
			if (ur->has_priority) {
				...
				if (ur->result->len[i] < 3) {
					REDEBUG("%s - Invalid data returned", xt->inst->name);
					goto error2;
				}
				priority_vb = fr_value_box_alloc_null(ctx);
				if (fr_value_box_from_network(ctx, priority_vb, FR_TYPE_UINT16, NULL, (uint8_t *)ur->result->data[i], 2, true) < 0) {
			...
			if (rrlabels_tovb(vb, ur->result->data[i] + offset) == XLAT_ACTION_FAIL) goto error2;
		}
			break;
		default:
			goto error2;
	...
	} while  ((++i < ur->count) && (ur->result->data[i]));
				  ...
				  fr_value_box_list_t *in)
{
	...
	ur = talloc_zero(request, unbound_request_t);

#define UB_QUERY(_record, _rrvalue, _return, _hasprio) \
	if (strcmp(query_vb->vb_strvalue, _record) == 0) { \
		ub_resolve_async(t->ub, host_vb->vb_strvalue, _rrvalue, 1, ur, link_ubres, &ur->async_id); \
	 ...
	 */
	if (fr_event_timer_in(ur, t->el, &ur->ev, fr_time_delta_from_msec(xt->inst->timeout), ub_timeout, ur) < 0) {
