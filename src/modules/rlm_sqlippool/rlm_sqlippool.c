...
static rlm_rcode_t CC_HINT(nonnull) mod_post_auth(void *instance, REQUEST *request)
{
	 ...
	 *	it is available then run that first
	 */
	if (inst->allocate_requested && *inst->allocate_requested &&
	...
	/*
	 *	If no IP found and we have a query to find an existing IP run that next
	 */
	if (allocation_len == 0 && inst->allocate_existing && *inst->allocate_existing) {
		allocation_len = sqlippool_query1(allocation, sizeof(allocation),
